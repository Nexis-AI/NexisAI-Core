...
pub const SUBCHAIN_CREATION_DEPOSIT_NZT: u64 = 1_000_000;
...
impl EvmProcessor {
            ...
            if subchain_feature && (fee_type.is_native() || subchain) {
                let (max_fee, min_lamports) = if subchain {
                    (
                        ...
                        SUBCHAIN_CREATION_DEPOSIT_NZT * LAMPORTS_PER_NZT,
                    )
                } else {
                    (tx_gas_price, 0)
                };
    ...
    fn process_swap_to_evm(
        ...
        if register_swap_tx_in_evm {
            executor.register_swap_tx_in_evm(*precompiles::ETH_TO_NZT_ADDR, evm_address, wei)
        }
        Ok(())
    }
    ...
    pub fn handle_subchain_transaction_result(
        ...
        ic_msg!(
                invoke_context,
                "Transaction executed with fee: in nzt {charge_from_native}, in subchain currency {return_to_zero_addr}",
        );
    ...
    fn create_evm_subchain_account(
            ...
            < SUBCHAIN_CREATION_DEPOSIT_NZT * LAMPORTS_PER_NZT
        {
            ic_msg!(
                invoke_context,
                "Deposit of {} NZT is required to create EVM Subchain",
                SUBCHAIN_CREATION_DEPOSIT_NZT
            );
        ...
        ic_msg!(
            invoke_context,
            "Creating subchain account, with transfering {SUBCHAIN_CREATION_DEPOSIT_NZT} NZT from {} to {}",
            whale_pubkey,
            evm_subchain_state_pubkey
        );
        invoke_context
...
mod test {
    ...
    #[test]
    fn swap_evm_to_nzt_within_different_chains() {
        // prepare context
        ...
        subchain_owner_acc.set_lamports(SUBCHAIN_CREATION_DEPOSIT_NZT * LAMPORTS_PER_NZT + 777);
    ...
    fn transfer_on_subchain(
        ...
        assert!(
            subchain_state_account_before.lamports()
                > LAMPORTS_PER_NZT * SUBCHAIN_CREATION_DEPOSIT_NZT,
        ...
        let subchain_state_account = evm_context.native_account(subchain_pubkey);
        assert!(
            subchain_state_account.lamports() >= LAMPORTS_PER_NZT * SUBCHAIN_CREATION_DEPOSIT_NZT
        );
