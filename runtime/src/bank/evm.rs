...
mod evmtests {
    ...
    use {
        ...
        solana_evm_loader_program::{
            instructions::AllocAccount, precompiles::ETH_TO_NZT_ADDR,
            processor::SUBCHAIN_CREATION_DEPOSIT_NZT, scope::evm::lamports_to_wei,
        },
    ...
    fn create_evm_subchain_regular() {
        ...
        let tx = solana_evm_loader_program::processor::dummy_call(0).0;
        let receiver = tx.caller().unwrap();
        let (genesis_config, mint_keypair) = create_genesis_config(LAMPORTS_PER_NZT * 1_000_001);
        let mut bank = Bank::new_for_tests(&genesis_config);
        ...
        let account = bank.get_account(&mint_keypair.pubkey()).unwrap_or_default();
        assert_eq!(account.lamports(), LAMPORTS_PER_NZT * 1);
    ...
    fn swap_within_subchain() {
        ...
        let subchain_creation_fee = SUBCHAIN_CREATION_DEPOSIT_NZT * LAMPORTS_PER_NZT;
        let mint_lamports = subchain_creation_fee + 1_000 * LAMPORTS_PER_NZT;
        ...
        assert_eq!(
            get_wei_balance(&bank, subchain_id, *ETH_TO_NZT_ADDR),
            0.into()
        );
        ...
        let swap_within_subchain = {
            ...
            let evm_tx = evm::UnsignedTransaction {
                ...
                action: evm_state::TransactionAction::Call(*precompiles::ETH_TO_NZT_ADDR),
